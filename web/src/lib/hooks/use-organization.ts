...
export function organizationSolutionsKey(organizationId: string | undefined) {
    return ['api', organizationId === undefined ? null : `/api/v1/organizations/${organizationId}/solutions`] as const;
}
...
export function useOrganizationRoute(organizationSlug: string) {
    ...
    const solutionsQuery = useQuery({
        ...
        refetchInterval: (query) =>
            query.state.data?.some((solution) => solution.status === 'creating' || solution.deployment_pending) === true
                ? 5000
                : false,
        meta: { polling: true },
        retry: false,
