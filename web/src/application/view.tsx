...
export default function ApplicationView() {
    ...
    const { data: registeredPages, error } = useApiQuery<RuntimePage[]>('/pages.json', {
    ...
    const activePageStateKey = activePage ? `/pages.json\u0000${activePage.path}\u0000${routePath}` : '';
    ...
    useEffect(() => {
        ...
        if (firstTabPage.route) {
            navigate(applicationHref(firstTabPage.route, '/'), { replace: true });
        }
    ...
    useEffect(() => {
        ...
        runtimeContext.services.navigationBaseUrl = applicationHref('', '/');
        ...
        try {
            pageUrl = resolveRequestUrl('/', activePage.path);
    ...
    }, [activePage, activePageStateKey, activeRouteParams]);
    ...
    if (error) {
        ...
        if (!activePage) {
            ...
            activeFallback = <ErrorState message={visiblePageState.message} title="Unable to load this page" />;
        ...
        content = (
            <>
                {visiblePageState?.status === 'ready' ? (
                    <RenderXML ast={visiblePageState.ast} baseUrl="/" ctx={visiblePageState.runtimeContext} />
                ) : null}
                {activeFallback && (
                    <Center minHeight="calc(100vh - 14rem)" width="100%">
                        {activeFallback}
    ...
    return (
        <ApplicationLayout basePath="/" pages={registeredPages ?? []}>
            {content}
        </ApplicationLayout>
    );
}
