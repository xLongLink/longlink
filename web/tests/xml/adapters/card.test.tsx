...
describe('Card', () => {
    ...
    it('preserves the simplified card structure in compiled xml', () => {
        expect(parseXML('<Card size="sm"><P i18n="Card Content" /></Card>')).toEqual([
            {
                name: 'Card',
                params: { size: 'sm' },
                children: [{ name: 'P', params: { i18n: 'Card Content' }, children: [] }],
            },
        ]);
    });
    ...
    it('renders the simplified card composition', () => {
        const output = renderXmlToMarkup(parseXML('<Card size="sm"><P i18n="Card Content" /></Card>'));
