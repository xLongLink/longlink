...
@pytest.mark.parametrize(
    "directives",
    [
        pytest.param([MigrationScript(None, UpgradeOps(), DowngradeOps())], id="empty-revision"),
        pytest.param([], id="no-directives"),
    ],
)
def test_make_migrations_skips_nonproductive_autogeneration(tmp_path, monkeypatch, directives: list[MigrationScript]) -> None:
    """Skip migration creation without generated schema operations."""
...
def test_make_migrations_keeps_nonempty_autogenerated_revisions(tmp_path, monkeypatch) -> None:
    """Write an Alembic revision when autogenerate finds schema operations."""

    # Arrange
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(database_migrations, "load_application_models", lambda: None)
    script = MigrationScript(None, UpgradeOps(ops=[CreateTableOp("inventory", [])]), DowngradeOps())
    directives = [script]
    ...
    def fake_revision(
        ...
        """Run Alembic's callback with one detected schema operation."""
    ...
    # Assert
    assert migration_created is True
