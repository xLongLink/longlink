...
SCHEMA = ROOT / ".static" / "xsd" / "layout" / "State.xsd"


def test_state_layout_validation() -> None:
    """Validate a minimal `State` layout fragment."""

    element = Element.from_content('<State id="filters"><query /></State>', schema=SCHEMA)
    element.validate()
...
def test_state_layout_requires_id() -> None:
    ...
    element = Element.from_content("<State />", schema=SCHEMA)
