...
- Attachments upload, list, download, and delete files through the Application filesystem in `src/resources.py`.
- `src/resources.py` scopes Application and shared filesystem views beneath one physical Organization bucket.
- The dashboard loads the Organization logo through `longlink.assets.logo()`, using the SDK fallback locally and the Organization shared filesystem in production.
