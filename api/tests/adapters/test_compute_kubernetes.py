...
async def test_setup_does_not_bootstrap_gateway(monkeypatch) -> None:
    """Leave cluster bootstrap to the external Kong installation."""
    ...
    class FakeResources:
        def get(self, *, api_version: str, kind: str):
            raise AssertionError(f"setup should not apply {api_version} {kind}")
