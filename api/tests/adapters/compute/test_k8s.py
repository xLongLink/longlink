...
k8s_module = importlib.import_module("src.adapters.compute.k8s")
...
async def test_application_applies_service_resources(monkeypatch) -> None:
    ...
    monkeypatch.setattr(k8s_module.client, "Configuration", FakeConfiguration)
    monkeypatch.setattr(k8s_module.config.kube_config, "KubeConfigLoader", FakeLoader)
    monkeypatch.setattr(
        k8s_module.client, "ApiClient", lambda configuration: type("ApiClient", (), {"configuration": configuration})()
    )
    monkeypatch.setattr(k8s_module.client, "CoreV1Api", lambda api_client: FakeCoreApi())
    monkeypatch.setattr(k8s_module.client, "AppsV1Api", lambda api_client: FakeAppsApi())
    adapter = K8s("apiVersion: v1\nclusters: []\n", "shared-secret")

    # Act
    result = await adapter.application(
        "acme", "dashboard", "ghcr.io/longlink/dashboard:latest", 80, {"REQUIRED": "value"}
    )
...
async def test_proxy_passes_json_body_as_decoded_value(monkeypatch) -> None:
    ...
    monkeypatch.setattr(k8s_module.client, "Configuration", lambda: object())
    monkeypatch.setattr(k8s_module.config.kube_config, "KubeConfigLoader", FakeLoader)
    monkeypatch.setattr(k8s_module.client, "ApiClient", FakeApiClient)
    monkeypatch.setattr(k8s_module.client, "CoreV1Api", lambda api_client: object())
    monkeypatch.setattr(k8s_module.client, "AppsV1Api", lambda api_client: object())
    adapter = K8s("apiVersion: v1\nclusters: []\n", "shared-secret")
