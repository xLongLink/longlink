...
async def test_create_allows_duplicate_application_names_across_organizations() -> None:
    ...
    # Act
    first_app = await db.applications.create(first_org.id, "dashboard", slug="dashboard", image="ghcr.io/longlink/dashboard:latest", user=user)
    second_app = await db.applications.create(second_org.id, "dashboard", slug="dashboard", image="ghcr.io/longlink/dashboard:latest", user=user)
