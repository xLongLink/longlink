...
async def proxy_root(req: Request, app_name: str):
    ...
    try:
        upstream = await apps.request(
            app.id, req.method.upper(), "pages", params=query_params
        )
...
async def proxy_path(req: Request, app_name: str, full_path: str):
    ...
    try:
        json_payload = None
        if req.method.upper() == "POST" and req.headers.get(
            "content-type", ""
        ).startswith("application/json"):
            json_payload = await req.json()

        upstream = await apps.request(
            app.id, req.method.upper(), path, params=query_params, json=json_payload
        )
