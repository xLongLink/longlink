...
| gatewayApi.listeners.tls.backendPort            | int          | `null`                                     | Backend service port that terminates TLS; defaults to the endpoint port.                                                                                                                                                                                                         |
...
RustFS supports [Gateway API](https://gateway-api.sigs.k8s.io/) through Traefik, Contour, and Istio gateway classes. Set `gatewayApi.enabled` to `true` to create the `Gateway` and `HTTPRoute` resources.
