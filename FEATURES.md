...
| Compute registries       | Creates Kubernetes-only compute registries with kubeconfig and a private API-facing gateway URL; lists, gets, and deletes unused registries for support/admin users. |
...
| Application gateway                 | Routes runtime traffic through the API proxy, which authorizes users and forwards requests to the assigned compute registry gateway URL with secret-protected per-cluster Envoy routing before internal service forwarding. |
...
| Gateway header policy               | Requires compute gateway secrets stored in Kubernetes Secrets, rejects unavailable apps with no-store 503, forwards only minimal content headers, and injects trusted `x-user-id` from the API proxy.                           |
...
| Feature                                   | Supported behavior                                                                                                                                                                                                                                                      |
| ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Operation records                         | Stores operation kind, app/org references, scheduled time, timestamps, broadly redacted bounded errors, lease metadata, and derived status.                                                                                                                             |
...
| Application verification operation        | Supports `application.verify` for checking rollout health.                                                                                                                                                                                                              |
| Application cleanup operation             | Supports `application.remove` for deleting managed app runtime resources.                                                                                                                                                                                               |
| Organization cleanup operation            | Supports `organization.remove` for deleting managed organization runtime resources.                                                                                                                                                                                     |
...
| Kubernetes gateway                        | Installs and synchronizes a private per-cluster Envoy gateway with Secret-backed route access, health probes, access logs, local rate limits, a ClusterIP Service plus development Ingress, and routes to managed ClusterIP app services. |
